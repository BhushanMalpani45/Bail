  ...
  useEffect(() => {
    const fetchCases = async () => {
      ...
      const response = await axios.post(
        "http://localhost:3000/prisoner/caseDetailsByPrisonerId",
        { prisonerId },
        {
        ...
        <div className="bg-gradient-to-r from-blue-50 via-white to-blue-50 border-2 border-[#03346E] p-8 rounded-lg shadow-lg hover:shadow-2xl transition-shadow duration-300 ease-in-out">
          ...
          <div className="flex justify-between">
            <p className="text-[#1E40AF] text-lg mb-4">
              <strong className="font-semibold">Case ID:</strong>{" "}
              {caseDetail.caseId}
            </p>
            <p className="text-[#1E40AF] text-lg mb-4">
              <strong className="font-semibold">Date FIled:</strong>{" "}
              {caseDetail.dateFiled.substr(0, 10)}
            </p>
          </div>
          ...
          <ul className="list-disc list-inside mb-6 text-[rgb(30,64,175)]">
            {caseDetail.length > 0 &&
              caseDetail.caseDetails.map((detail, index) => (
                <li
                  key={index}
                  className="mb-3 bg-gray-100 p-4 rounded-lg transition-transform duration-200 ease-in-out transform hover:scale-105"
                >
                  <strong className="font-semibold">{detail.title}:</strong>{" "}
                  {detail.description}
                  <br />
                  <em className="text-sm text-gray-600">Penalty:</em>{" "}
                  {detail.sentenceDetails.duration}
                </li>
              ))}
          </ul>

          <p className="text-[#1E40AF] text-2xl mb-4">
            <strong className="font-semibold">Imprisonment Duration:</strong>{" "}
            {caseDetail.sentenceDetails.duration}
          </p>
          <p className="text-[#1E40AF] text-2xl mb-4">
          ...
          <p className="text-[#1E40AF] text-2xl mb-6 mt-4">
            <strong className="font-semibold">Judge:</strong>{" "}
            {caseDetail.judge.name}
          </p>

          <h3 className="text-2xl font-semibold text-[#1E40AF] mb-4  mr-1">
            Case Status:
            <span className="text-[#DC2626] text-xl ml-1 mb-6 font-semibold">
              {caseDetail.status}
            </span>
          </h3>
